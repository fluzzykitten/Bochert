...
public class node {
	...
	public node create_nodes(int[] values){
		...
		node temp2, temp = new node(values[0]);		
		next = temp;
		previous =temp;		

		for (int i = 1; i<values.length ; i++){
			temp2 = new node(values[i]);
			temp.set_next(temp2);
	...
	public void verbose_test(){
		String list = "";
		node center = next;
		node back = next.previous;
		node forward = next.next;
		int[] error = new int[1];
		int real_length = 1;
	...
	public node split_nodes(int[] split_int){ //remove out listed nodes into seperate node list
		node split_head = new node();
		node temp_main_1=next, temp_main_2; 
		node temp_split_head = split_head, temp_split_tail = split_head;
		int i = 0;
	...
	public boolean insert_single_node_infront_of_this_node(node head){
	...
	public boolean add_unordered(node head){
		if ((head.length == 1) && (head.node == -1)){
			return true;
		}
	...
	public boolean add(node head){ // put two node lists together, orderly
		node point = next;
		node temp_next;
		length = length + head.length -1;
...
//			}
	...
	public int[] combine(node head){ //put the two lists together and get array of length combined size
		if (head == null)
